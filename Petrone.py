...
fill((160,132,173))
def main():
  global screen,HEIGHT,WIDTH,colour,pim,SEG,FONT,white_count,dir_t,dir_r,disable_time,petrone_health,seg_health
  pygame.init()
  ...
  while True:
    ...
    if not BOSSIN:
      ...
      move_window(POS[0]+round(math.cos(math.radians(SEG%360))*20),POS[1])
      seg.pos.right += R
      seg.pos.top += Y
    ...
    #change window pos?
    if k[pygame.K_0]:
      for i,e in os.environ.items():
        print(i+" - "+e)
      os.environ['SDL_VIDEO_WINDOW_POS']= '100,100'
      screensize(WIDTH+1,HEIGHT)
